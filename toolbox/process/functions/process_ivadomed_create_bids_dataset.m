...
function sProcess = GetDescription() %#ok<*DEFNU>
    ...
    sProcess.Comment     = 'Ivadomed Create BIDS EEG Dataset';
    ...
    sProcess.options.BIDSfolder.Comment = 'Parent folder to store mat dataset in:';
...
%     
    ...
    sProcess.options.segment_number.Comment = 'Maximum number of IEDs trials';
    ...
    sProcess.options.baselines.Comment = {'Do not include non IEDs trials', 'Include the same number of IEDs and non IEDs trials', 'Include all non IEDs trials'};
    sProcess.options.baselines.Type    = 'radio';
    sProcess.options.baselines.Value   = 1;
...
%     modality = sProcess.options.modality.Comment{sProcess.options.modality.Value};
    ...
    for iInput = 1:length(sInputs)
        ...
        if ~any(areSelectedEventsPresent) & sProcess.options.baselines.Value < 3
...
%                     end
    ...
    for isub = 1:length(subjects_name)
        ...
        if sProcess.options.baselines.Value == 2
...
%     
    ...
    if isempty(sProcess.options.BIDSfolder.Value{1})
        ...
        maxID = 0;
        for iFolder = iDirs
            if strfind(files(iFolder).name, [protocol.Comment '_datasetID_'])
        ...
        % one
        if maxID==0
            parentPath = bst_fullfile(sProcess.options.BIDSfolder.Value{1}, [protocol.Comment '_datasetID_1']);
        end
    end
...
%     
    ...
    for iInput = 1:length(sInputs)
        ...
        subject = lower(str_remove_spec_chars(sInputs(iInput).SubjectName));
        session = lower(str_remove_spec_chars(sInputs(iInput).Condition));
        ...
        trial = [comment iEpoch];

        OutputFile = bst_fullfile(['sub-' subject], ['ses-' session], ['sub-' subject '_ses-' session '_trial-' trial '_eeg.edf']);
        OutputFile_full = bst_fullfile(parentPath, OutputFile);
 
        
        InputFile_full = bst_fullfile(protocol.STUDIES, FileName);
        % Create the output folder first
        [filepath,name,ext] = fileparts(OutputFile_full);
        if ~exist(filepath, 'dir')  % This avoids a warning if the folder already exists
            mkdir(bst_fileparts(OutputFile_full))
        end   
        
        ChannelFile = bst_get('ChannelFileForStudy', InputFile_full);
        ChannelMat = in_bst_channel(ChannelFile);
        
        DataMat = in_bst(InputFile_full);

        selectedChannels = [];

        for iChannel = 1:length(ChannelMat.Channel)
           if strcmp(ChannelMat.Channel(iChannel).Type, 'EEG') && ...
               DataMat.ChannelFlag(iChannel)==1
               selectedChannels(end + 1) = iChannel;
           end
        end
        DataMat.F = DataMat.F(selectedChannels,:);
        DataMat.ChannelFlag = DataMat.ChannelFlag(selectedChannels,:);
        ChannelMat.Channel = ChannelMat.Channel(:, selectedChannels);
        
        [filepath,name,ext] = fileparts(InputFile_full);
           
        export_data(DataMat, ChannelMat, OutputFile_full, 'EEG-EDF');
